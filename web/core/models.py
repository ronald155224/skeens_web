...
class Setting(models.Model):
    ...
    email = models.CharField(
        'EMAIL', default='', null=True, max_length=100, blank=True
    )
    address = models.CharField(
        '地址', default='', null=True, max_length=100, blank=True
    )
...
class SlideImage(models.Model):
    name = models.CharField(_('slideimage_name'), max_length=50)
    setting = models.ForeignKey('core.Setting', on_delete=models.CASCADE, related_name='setting_slideimage_set')
    image = models.ImageField(_("image"), null=True, blank=True, upload_to=helpers.upload_handle)
    order = models.PositiveIntegerField(null=True, blank=True)

    class Meta:
        verbose_name = _('Slideimage')
        verbose_name_plural = _('Slideimage')
        ordering = ['order']
